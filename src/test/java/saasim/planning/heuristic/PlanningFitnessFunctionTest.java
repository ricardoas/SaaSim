...
public class PlanningFitnessFunctionTest extends MockedConfigurationTest {
	...
	public void testCalculateCostWith2MachineTypesAndNoOnDemandResources(){
		...
		double expectedCost = 100 * 15 + 99 * 8 + 432 + 6480;//One year fee for large + one year fee for medium + large usage + medium usage
	...
	public void testCalculateCostWith2MachineTypesAndOnDemandResources(){
		...
		double expectedCost = 100 * 15 + 99 * 8 + 216 + 3240 + 0.6;//One year fee for large + one year fee for medium + large usage + medium usage + on-demand cost
	...
	public void testCalculateCostWith2MachineTypesAndOnDemandResources2(){
		...
		double expectedCost = 100 * 15 + 99 * 8 + 216 + 3240 + 17280;//One year fee for large + one year fee for medium + large usage + medium usage + on-demand cost
		assertEquals(expectedCost, function.calcCost(finishedRequestsPerMachineType, meanServiceTimeInMillis, currentPowerPerMachineType, 
				0, 622080900), 0.00001);
	...
	public void testCalculateCostWithOnlyOneThroughput(){
		...
		double expectedCost = 100 * 15 + 99 *8 + 143.9986;//One year fee for large + one year fee for medium + large usage + medium usage
		assertEquals(expectedCost, function.calcCost(finishedRequestsPerMachineType, meanServiceTimeInMillis, currentPowerPerMachineType, 
				0, 0), 0.001);
	...
	public void testCalculateCostWithOnlyOneThroughputAndOnDemandResources(){
		...
		double expectedCost = 100 * 15 + 99 *8 + 143.9986 + 2160;//One year fee for large + one year fee for medium + large usage + medium usage
		assertEquals(expectedCost, function.calcCost(finishedRequestsPerMachineType, meanServiceTimeInMillis, currentPowerPerMachineType, 
				0, 5 * 31104000), 0.001);
...
//		EasyMock.expect(config.getParserPageSize()).andReturn(TickSize.MINUTE);
		...
		double cost = largeReservation * 15 + 1488 * 0.12 + mediumReservation * 5 + 1488 * 0.06 + 8183.997777777778 * 0.085;//for each machine type: reservation fee + usage
		double penalties = 0;//Since loss is more than 5%, SaaS client does not pay the provider
